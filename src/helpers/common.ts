...
export async function updateData(id, data) {
  ...
  const obj = candidateName.find((o) => o.name)
  const name = obj.name
  const email = obj.email
  let update =
    'SET #c_name = :name, email = :email, first_slot = :first_slot, second_slot = :second_slot, third_slot = :third_slot, updated_at = :updated_at'
  let value = {
    ':name': name,
    ':email': email,
  ...
  if (
    data.first_slot === 'available' ||
    data.second_slot === 'available' ||
    data.third_slot === 'available'
  ) {
  ...
  if (
    data.first_slot === 'booked' &&
    data.second_slot === 'booked' &&
    data.third_slot === 'booked'
  ) {
    update += ', availability = :availability'
